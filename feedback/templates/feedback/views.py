...
LESSONS = build_semester2() + LESSONS

# ===== 학생정보 검증 =====
import re as _re

def validate_student(lesson, student_id, student_num, student_name):
    if not _re.fullmatch(r'\d{5}', student_id):
        return '학번은 숫자 5자리로 입력해주세요. (예: 10113)'
    if not student_num.isdigit() or not (1 <= int(student_num) <= 30):
        return '번호는 1~30 사이 숫자로 입력해주세요.'
    if int(student_id[3:5]) != int(student_num):
        return f'학번 뒤 2자리({student_id[3:5]})와 번호({student_num})가 일치하지 않습니다.'
    class_num = int(_re.sub(r'[^0-9]', '', lesson['class']))
    if int(student_id[1:3]) != class_num:
        return f'학번의 반 번호({student_id[1:3]})가 이 수업({lesson["class"]})과 일치하지 않습니다.'
...
def feedback_create(request, lesson_id):
    ...
    if request.method == 'POST':
        ...
        # 학생정보 검증
        v_error = validate_student(lesson, student_id, student_num, student_name)
        if v_error:
...
_VALID_IDS = set(
    f'{cls}{num:02d}'
    for cls, count in [('101', 30), ('102', 30), ('103', 30), ('104', 30), ('105', 30),   # 2학기 1~5반
                       ('108', 30), ('109', 30), ('110', 30),
                       ('111', 30), ('112', 29), ('113', 29)]                               # 1학기 8~13반
    for num in range(1, count + 1)
)

# lesson_id 앞자리 → 반 코드 매핑
_LESSON_CLASS_CODE = {10: '101', 20: '102', 30: '103', 40: '104', 50: '105',   # 2학기 (id 1001~5999)
                      1: '108', 2: '109', 3: '110', 4: '111', 5: '112', 6: '113'}  # 1학기 (id 101~699)
